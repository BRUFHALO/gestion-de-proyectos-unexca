...
export function StudentPDFViewer({ projectId, onBack }: StudentPDFViewerProps) {
  ...
  const [gradeData, setGradeData] = useState<{ grade: number; grade_type: string; status: string } | null>(null);
  ...
  const loadStudentData = () => {
    try {
      ...
      // Cargar información del profesor asignado
      if (userData.assigned_teacher) {
        setTeacherInfo({
          id: userData.assigned_teacher.teacher_id || '',
          name: userData.assigned_teacher.teacher_name || 'Profesor'
        });
      }
    } catch (error) {
  ...
  const loadPDF = async () => {
    ...
    try {
      ...
      // Guardar información del proyecto para el chat
      setProjectInfo({
  ...
  const loadGrade = async () => {
    try {
      if (!projectId) {
        console.warn('No hay projectId para cargar calificación');
        return;
      }
      console.log('Cargando calificación para proyecto:', projectId);
      const response = await fetch(
        `${API_BASE_URL}/api/v1/projects/${projectId}/evaluation/grade`
      );
      ...
      if (response.ok) {
        ...
        console.log('No hay calificación guardada aún');
        setGradeData(null);
      }
    } catch (error) {
    ...
    <div className="flex h-screen bg-slate-100">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  console.log('Recargando anotaciones y calificación manualmente...');
                  loadAnnotations();
                  loadGrade();
                }}
      ...
      {teacherInfo && studentData && (
        <button
          ...
          title={`Chat con ${teacherInfo.name}`}
        >
          <MessageSquare className="w-6 h-6" />
        </button>
      )}
