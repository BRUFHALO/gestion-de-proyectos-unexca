  ...
  const loadProjectDetails = async () => {
    try {
      ...
      if (gradeResponse.ok) {
        ...
        // Combinar datos del proyecto con la calificación
        const combinedData = {
          ...projectData,
          grade: gradeData.grade,
          grade_type: gradeData.grade_type,
          status: gradeData.status || 'en_revision',
        ...
        console.log('No grade data found, using project data only');
        // Si no hay calificación, mostrar solo los datos del proyecto
        setProject({
          ...projectData,
          grade: null,
          status: 'en_revision'
                  ...
                  {comment.resolved !== undefined && (
                    <span
                      className={`text-xs font-medium ${
                        comment.resolved ? 'text-green-600' : 'text-yellow-600'
                      }`}
                    >
                    ...
                    {comments
                      ...
                      .map((comment, index) => (
                        <div
                          ...
                          } ${activeCommentId === comment.id ? 'ring-2 ring-primary ring-offset-2' : ''}`}
                        >
